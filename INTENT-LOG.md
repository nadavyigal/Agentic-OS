...
## 2026-07-20 - Stop the manual dashboard layer from rotting under a fresh timestamp
**Request:** Clear the Portfolio HQ trust blocker without deleting any branch, audit the Launch Pad cards that were never individually verified, fix the root cause behind the stale-claim rot rather than the claims themselves, and document a PostHog project trap that nearly produced a false outage report.
**Intent / why:** The rot had a structural cause, not a discipline cause: `portfolio-hq-manual.json` is hand-maintained input the refresh never writes, so it decayed silently while the page stamped `asOf` as today. A current-looking date over stale content is worse than an obviously old page because it defeats the reader's own staleness instinct, which is why 47 stale-claim matches accumulated unnoticed. The durable fix was to give every manual block its own last-verified date and route anything older than a week into the existing contradiction surface, so decay downgrades trust instead of hiding. Verification kept beating prose again: the "2 open worktrees" in the packet were actually 4, two Launch Pad cards still described work that shipped a release ago, and two readouts still said "in review" for builds Apple had already released. Worth noting the constraint this session could not resolve from inside: a session running in a worktree cannot remove its own worktree, so the trust panel is structurally unable to read actionable while the work that clears it is in flight.
**Themes:** trust, verifiability, external-ground-truth, silent-decay, process-drift
