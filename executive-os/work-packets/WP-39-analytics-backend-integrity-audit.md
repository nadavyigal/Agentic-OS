...
## S4 — status: OPEN, verdict not accepted yet (2026-07-09)
Cursor traced the real `applied_at` writer (`applyExpertWorkflowRun()`), the actual UI call sites (web `ExpertModesPanel`, iOS `ExpertModesView`), and git history around the cliff — solid, verifiable investigation. Its verdict ("scenario a: Apply exists and is reachable, users just aren't finishing the second step, not a regression") is not accepted as final: the underlying data shows applies going from ~60% completion in the first two weeks (8, then 2 applies) to **exactly zero across the following four months and ~110 runs** — a hard cliff, not a gradual UX drop-off, and it lands almost exactly on a 2026-03-09 commit that "hardened apply response shape." Cursor confirmed that commit didn't remove the UI/route, but did not confirm the frontend still correctly handles the changed response shape. The decisive missing evidence (`expert_apply_clicked` PostHog volume) was flagged by Cursor itself as unqueried, and is still unreachable from any session this WP has used (no PostHog query tool available). **Next step: live smoke test** using the existing Resumely QA account (`nadav.yigal+fable-qa-jul03@gmail.com`) — run a real optimization through Expert Modes, click Apply, and confirm in Supabase whether `applied_at` actually gets set today. That single test resolves scenario (a) vs a live regression.
