...
Last Validation: Models-tab change (2026-07-10) — embedded payload JSON re-parsed clean (10 models / 4 harnesses / 13 matrix rows / spend); `node --check` on the page JS passed; Models tab rendered + screenshot-verified in a headless Chrome (recommendation banner, utilities, pricing table with tier dots, routing matrix, spend bars). `refresh_portfolio_hq.py` and `collect_usage.py` both exit 0. Product PRs #117/#113 merged (RunSmart CLEAN; ResumeBuilder merged --admin past the known-failing Cloudflare check). Both product repos restored to their original branches.
