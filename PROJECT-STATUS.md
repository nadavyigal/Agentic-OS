...
| Agentic OS | Advanced OS patterns lean pilot | Finish dashboard-trust reconciliation, push Agentic OS main, then use the refreshed one-move recommendation for today's work | 0 | Yes (16) | Fresh | Medium | tasks/progress.md | 2026-07-17 db34d432 Merge dashboard FTUX submissions and artifacts |
...
- Agentic OS: validated 2026-07-13, last commit 2026-07-17 db34d432 Merge dashboard FTUX submissions and artifacts
...
- [Resumely iOS] main is 15 commit(s) behind origin (pull needed) -> Sync the default branch first: pull, then push.
...
- [Resumely iOS] worktree on detached at /Users/nadavyigal/Documents/Projects /ResumeBuilder/ResumeBuilder IOS APP/.claude/worktrees/resumely-release-a-b-landing-325d9a -> Land or discard this worktree, then `git worktree remove` it.
...
- RunSmart iOS: **Investigate the sign-in P0 (below) before 2026-07-22.** Then, if E1 is still wanted, build E1 exposure/assignment instrumentation — it does not exist on `main` (searched: no flag read, no exposure event, no arm allocation; the only `variant` hit is a hardcoded `"C"` literal in AhaMoments). Re-run the frozen mature-D7 query on **2026-07-22** (`REPLACE_SNAPSHOT_END_UTC` ≥ `2026-07-22 09:30:00`) — a maturity checkpoint, NOT a decision date (denominator will be ~2 vs the 10 minimum)
- Resumely iOS: Re-run PostHog picker→file-selected funnel on **2026-07-25** (or minimum check **2026-07-18**) for clean `marketing_version=1.4.1` cohort; see deferred-read entry above for query definition
...
- RunSmart iOS: 27 saved plan(s) — latest "RunSmart Adaptive Preview — Side-by-Side Experiment" (2026-07-16). See Saved Plans.
...
- Resumely iOS: docs/specs/drafts/first-time-user-journey-upgrade-figma-board.md
...
- Resumely iOS: worktree:claude/release-b-story-9:docs/specs/drafts/recommendation-evidence-backend-contract.md
